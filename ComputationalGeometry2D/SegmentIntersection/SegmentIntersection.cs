...
namespace ComputationalGeometry2D.SegmentIntersection
{
    class SegmentIntersection
    {
        ...
        public List<Intersection> SweepLine(IEnumerable<LineSegment> segments)
        {
            ...
            TreeSet<EventPoint> eventQueue = GetEventQueue(segments);
            ...
            while (eventQueue.Count > 0)
            {
                ...
                TryMarkEventPointAsIntersection(eventPoint, result);
                ...
                TryFindNewEventInEventPointVicinity(eventPoint, sweepLine, eventQueue);
        ...
        private TreeSet<EventPoint> GetEventQueue(IEnumerable<LineSegment> segments)
        {
            TreeSet<EventPoint> eventQueue = new TreeSet<EventPoint>(new EventPointComparer());
        ...
        private TreeSet<LineSegment> distinctEventSegments = new TreeSet<LineSegment>(new SegmentsFromTopToBottomComparer());

        private void TryMarkEventPointAsIntersection(EventPoint eventPoint, List<Intersection> result)
        {
            ...
            if (upperCount + intersectionCount + downCount > 1)
            {
                ...
                if (distinctEventSegments.Count > 1)
                    result.Add(new Intersection(eventPoint.PointValue, distinctEventSegments.ToArray()));
            }
        }

        private List<LineSegmentNode> currentEventUpperForAndIntersectionFor = new List<LineSegmentNode>();

        private void TryFindNewEventInEventPointVicinity(EventPoint eventPoint, TreeSet<LineSegmentNode> sweepLine, TreeSet<EventPoint> eventQueue)
        {
            if (eventPoint.UpperFor.Count + eventPoint.IntersectionFor.Count == 0)
            {
